...
class AdminTestimonialsController extends ModuleAdminController
{
  ...
  private $_idLang;
  private $_obj;
  protected $position_identifier = 'id_testimonials';
  ...
  public function __construct()
  {
    ...
    $this->position_identifier = 'id_testimonials';
    $this->_defaultOrderBy = 'a!position';
    $this->orderBy = 'position';
    $this->_imgDir = _PS_MODULE_DIR_ . 'mpm_testimonials/views/img/';
    $this->_idShop = Context::getContext()->shop->id;
    $this->_idLang = Context::getContext()->language->id;
    $this->_obj = new testimonials();
  ...
  {
    parent::init();
    if (Shop::getContext() == Shop::CONTEXT_SHOP && Shop::isFeatureActive() && Tools::getValue('viewtestimonials') === false)
  ...
  {
    $settings = Configuration::get('GOMAKOIL_TESTIMONIALS');
    $form = $this->getFormGeneralSettings($settings);
    $this->tpl_list_vars['form'] = $form;
    $this->tpl_list_vars['token_admin'] = Tools::getAdminTokenLite('AdminTestimonials');
    parent::initContent();
  }
  ...
  public function getFormGeneralSettings($settings){
    ...
    $this->fields_value['token_slider'] = Tools::getAdminTokenLite('AdminTestimonials');
  ...
  {
    $res = $this->uploadImage($id, 'image', $this->_imgDir, 'png', 120, 120);
    return $res;
  }
  ...
  {
    $product_info = Tools::getValue('testimonials');
    foreach($product_info as $key => $value){
      $value = explode('_', $value);
      Db::getInstance()->update('testimonials', array('position' => (int)$key), 'id_testimonials='.(int)$value[2]);
    }
  ...
  public function displayAjax()
  {
    ...
    try{
      if (Tools::getValue('action') == 'saveCustom'){
        Configuration::updateValue('GOMAKOIL_TESTIMONIALS', Tools::getValue('hook'));
        $json['success'] = Module::getInstanceByName('mpm_testimonials')->l("Successfully saved!") ;
      }
